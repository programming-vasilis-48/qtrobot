...
class FeatureExtractorNode:
    ...
    def process_features(self, features):
        ...
        # Create a new FaceFeatures message to avoid modifying the original
        processed_features = FaceFeatures()
        
        # Copy all fields from the original message
        processed_features.header = features.header
        processed_features.bbox = features.bbox
        processed_features.detection_confidence = features.detection_confidence
        processed_features.landmarks = features.landmarks
        
        # Copy action units
        processed_features.action_units = list(features.action_units)
        ...
        if len(processed_features.action_units) >= 3:
            # Make a copy of the action_units list to avoid modifying a tuple
            action_units = list(processed_features.action_units)
            action_units[2] = min(1.0, action_units[2] * 1.2)  # AU04
            processed_features.action_units = action_units
        
        # Copy head pose and eye gaze
        processed_features.head_pose = features.head_pose
        processed_features.left_eye_gaze = features.left_eye_gaze
        processed_features.right_eye_gaze = features.right_eye_gaze
